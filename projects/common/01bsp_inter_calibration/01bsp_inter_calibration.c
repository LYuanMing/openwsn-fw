...
#define LENGTH_WITHOUT_CRC 4
#define LENGTH_PACKET LENGTH_WITHOUT_CRC + LENGTH_CRC // maximum length is 127 bytes
...
#define TICKS_OF_PERIODICAL_BEACON 8192 // 3277 = 100ms @ 32kHz
...
char offset[] = "+00\r\n";
uint32_t packet_idx;
char packet_idx_str[] = "0000000000\r\n";
//=========================== prototypes ======================================
        ...
        switch (app_vars.state)
        {
            case S_SEND_BEACON:
                ...
                if (app_vars.txpk_txDone)
                {
                    ...
                    {
                        length = 34;
                        send_string("next status: S_PERIODICAL_BEACON\r\n");
                        app_vars.timer_triggered = 1;
                        app_vars.state = S_PERIODICAL_BEACON;
                        break;
                    }
            ...
            //     }
            // break;
            case S_PERIODICAL_BEACON:
                if (app_vars.txpk_txDone == 1 && app_vars.timer_triggered == 1)
                {
                    // length = 5;
                    // send_string(offset);
                    length = 13;
                    send_string("send beacon\r\n");
                    app_vars.txpk_txDone = 0;
                    // led
                    leds_error_toggle();
                    ...
                    radio_txNow();
                    // start a timer
                    app_vars.timer_triggered = 0;
                    sctimer_setCompare(sctimer_readCounter() + TICKS_OF_PERIODICAL_BEACON);
                    // and then start to listening the packet from scum
                }
                break;
            case S_LISTENING:
                if (app_vars.txpk_txDone == 1 && app_vars.rxpk_rxDone == 1)
                {
                    length = 11;
                    ...
                    while (app_vars.timer_triggered == 0) {
                        if (app_vars.rx_open == 0) {
                            radio_rfOff();
                            radio_rxEnable();
                            radio_rxNow();
                            delay();
                        ...
                        if (app_vars.rxpk_rxDone == 1) {
                            temp_packet_idx = packet_idx;
                            for (int i = 1; i < 10; i++) {
                                packet_idx_str[10 - i] = '0' + (temp_packet_idx % 10);
                                temp_packet_idx /= 10;
                            }
                            length = 12;
                            send_string(packet_idx_str);
                            length = 5;
                            send_string(offset);
                            app_vars.rxpk_rxDone = 0;
                        }
...
void cb_scTimerCompare(void)
{
    ...
    else
    {
        ...
        else if (app_vars.state == S_LISTENING || app_vars.state == S_PERIODICAL_BEACON)
        {
            app_vars.timer_triggered = 1;
            app_vars.state = S_PERIODICAL_BEACON;
        }
...
void cb_endFrame(PORT_TIMER_WIDTH timestamp)
{
    ...
    else if (app_vars.state == S_PERIODICAL_BEACON) {
        app_vars.txpk_txDone = 1;
    ...
    else
    {
        if (app_vars.state == S_LISTENING)
        {
            radio_getReceivedFrame(
                ...
                &lqi,
                &crc);
            app_vars.rxpk_len = pkt_len;
            if (
                crc &&
                pkt_len == LENGTH_PACKET &&
                packet[0] == 'P')
            {
                // packet_idx_str[0] = packet[1];
                packet_idx = 0;
                // sctimer_disable();
                app_vars.rxpk_rxDone = 1;
                // packet_idx = (packet[1] << 24) + (packet[2] << 16) + (packet[3] << 8)+ (packet[4]);
                packet_idx = (packet[2] << 8) + packet[3];


                // report the offset of the transimitter
                freq_offset = radio_getFrequencyOffset();
                if(freq_offset < 0) {
                    offset[0] = '-';
                    freq_offset = -freq_offset;
                    offset[1] = freq_offset / 10 + '0';
                    offset[2] = freq_offset % 10 + '0';
                } else {
                    offset[0] = '+';
                    offset[1] = freq_offset / 10 + '0';
                    offset[2] = freq_offset % 10 + '0';
