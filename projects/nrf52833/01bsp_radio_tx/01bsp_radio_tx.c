...
#define LENGTH_BLE_CRC  2
#define LENGTH_PACKET   253+LENGTH_BLE_CRC
...
#define TIMER_PERIOD    (655)    // 655 = 20ms @ 32kHz
//#define TIMER_PERIOD    (1638)    // 1638 = 50ms @ 32kHz
#define ENABLE_DF       0
...
int mote_main(void) {
    ...
    sctimer_enable();
    for(i = 1; i<LENGTH_PACKET-LENGTH_BLE_CRC; i++) {
      app_vars.txpk_buf[i] = i;
    }
    app_vars.txpk_buf[0] = LENGTH_PACKET-LENGTH_BLE_CRC;
    while(1) {
