...
void isr_ieee154e_timer(opentimers_id_t id) {
  ...
  default:
    // log the error
    LOG_CRITICAL(COMPONENT_IEEE802154E,
...
port_INLINE void activity_synchronize_newSlot(void) {
  ...
  opentimers_scheduleAbsolute(ieee154e_vars.serialInhibitTimerId,    // timerId
...
port_INLINE void activity_synchronize_endOfFrame(PORT_TIMER_WIDTH capturedTime) {
  ...
  */
  do {    // this "loop" is only executed once
    ...
    // process IEs
    lenIE = 0;
    if ((ieee802514_header.valid == TRUE && ieee802514_header.ieListPresent == TRUE &&
    ...
    // log the info
    LOG_SUCCESS(
        COMPONENT_IEEE802154E, ERR_SYNCHRONIZED, (errorparameter_t)ieee154e_vars.slotOffset, (errorparameter_t)0);
...
port_INLINE void activity_ti1ORri1(void) {
  ...
  uint8_t neighbor_throttle_factor;
  ...
  if (idmanager_getIsDAGroot() == FALSE) {
    if (ieee154e_vars.deSyncTimeout > ieee154e_vars.numOfSleepSlots) {
      ...
      // log the error
      LOG_ERROR(
          COMPONENT_IEEE802154E, ERR_DESYNCHRONIZED, (errorparameter_t)ieee154e_vars.slotOffset, (errorparameter_t)0);
  ...
  // check whether the node should skip the slot
#if ENERGY_THROTTLE
  if (ieee154e_vars.slotOffset != 0) {
      // if this slot is what I should skip, then I skip
      // if this slot is what my neighbor skip, then I skip
      asn_low32 = (uint32_t)ieee154e_vars.asn.bytes0and1 | ((uint32_t)ieee154e_vars.asn.bytes2and3 << 16);
      current_sf = asn_low32 / schedule_getFrameLength();

      if (energy_vars.throttle_factor > 0) {
          throttol_mask = (1 << energy_vars.throttle_factor) - 1;
          if ((current_sf & throttol_mask) != 0) {
              openserial_printf("SF %d I skip slot\r\n", current_sf);
              endSlot();
              return;
          }
      }
      ...
      if (idmanager_getIsDAGroot() == TRUE) {
        neighbor_throttle_factor = 0;
      } else {
          schedule_getNeighbor(&neighbor);
          
          if (neighbor.type == ADDR_ANYCAST || neighbor.type == ADDR_NONE) {
              // it is Minimal Cell here
              cellType = schedule_getType();
              if (cellType == CELLTYPE_TX && ieee154e_vars.dataToSend != NULL) {
                  // If I got a packet to send, then I check whether I can send now
                  memcpy(&neighbor, &(ieee154e_vars.dataToSend->l2_nextORpreviousHop), sizeof(open_addr_t));
                  neighbor_throttle_factor = neighbors_getEnergyThrottleFactor(&neighbor);
              } else {
                  neighbor_throttle_factor = 0;
              }
          } else {
              // it is Dedicated Cell here
                neighbor_throttle_factor = neighbors_getEnergyThrottleFactor(&neighbor);
          }
      }

      if (neighbor_throttle_factor > 0) {
          throttol_mask = (1 << neighbor_throttle_factor) - 1;
          if ((current_sf & throttol_mask) != 0) {
              openserial_printf("SF %d neighbor skip slot\r\n", current_sf);
              endSlot();
              return;
          }
      }
  ...
  // check whether the neighbor skip the slot
  ...
  case CELLTYPE_TX:
    ...
    if (ieee154e_vars.dataToSend == NULL) {
      ...
      if (ieee154e_vars.localCopyForTransmission.l2_securityLevel !=
          IEEE154_ASH_SLF_TYPE_NOSEC) {    // security enabled
  ...
  */
  do {    // this "loop" is only executed once
    ...
    if (ieee154e_vars.ackReceived->length < LENGTH_CRC || ieee154e_vars.ackReceived->length > LENGTH_IEEE154_MAX) {
      // break from the do-while loop and execute the clean-up code below
      LOG_ERROR(
          COMPONENT_IEEE802154E, ERR_INVALID_PACKET_FROM_RADIO, (errorparameter_t)1, ieee154e_vars.ackReceived->length);
...
port_INLINE void activity_ri5(PORT_TIMER_WIDTH capturedTime) {
  ...
  do {    // this "loop" is only executed once
    ...
    ieee154e_vars.dataReceived->l2_timeCorrection =
        (PORT_SIGNED_INT_WIDTH)((PORT_SIGNED_INT_WIDTH)TsTxOffset -
                                (PORT_SIGNED_INT_WIDTH)ieee154e_vars.syncCapturedTime);
    ...
    if (ieee802514_header.ackRequested == 1 && ieee154e_vars.isAckEnabled == TRUE) {
      ...
      // calculate the time timeCorrection (this is the time the sender is off w.r.t to this node. A negative number
      // means the sender is too late.
      ieee154e_vars.timeCorrection = (PORT_SIGNED_INT_WIDTH)((PORT_SIGNED_INT_WIDTH)TsTxOffset -
                                                             (PORT_SIGNED_INT_WIDTH)ieee154e_vars.syncCapturedTime);
      ...
      radio_loadPacket_prepare(ieee154e_vars.ackToSend->payload, ieee154e_vars.ackToSend->length);
      ...
      if (idmanager_getIsDAGroot() == FALSE &&
          (icmpv6rpl_isPreferredParent(&(ieee154e_vars.dataReceived->l2_nextORpreviousHop)) ||
           IEEE802154_security_isConfigured() == FALSE || icmpv6rpl_getPreferredParentEui64(&addressToWrite) == FALSE ||
...
port_INLINE bool isValidAck(ieee802154_header_iht *ieee802514_header, OpenQueueEntry_t *packetSent) {
  return ieee802514_header->valid == TRUE && ieee802514_header->frameType == IEEE154_TYPE_ACK &&
      packetfunctions_sameAddress(&ieee802514_header->panid, idmanager_getMyID(ADDR_PANID)) &&
      idmanager_isMyAddress(&ieee802514_header->dest) &&
...
port_INLINE void incrementAsnOffset(void) {
  ...
  // if it is the end of slotframe, then we conduct the energy measurement
  if ((ieee154e_vars.slotOffset + 1) == frameLength) {
    scheduler_push_task(task_energyMeasurement, TASKPRIO_OPENTIMERS);
  }
}
...
bool isValidEbFormat(OpenQueueEntry_t *pkt, uint16_t *lenIE) {
  ...
  volatile uint8_t energy_throttle_factor;
  open_addr_t* neighbor_addr;
  neighborRow_t neighbor_row;
  ...
  while (ptr < pkt->length) {
    ...
    if ((temp16b & IEEE802154E_DESC_GROUPID_PAYLOAD_IE_MASK) >> IEEE802154E_DESC_GROUPID_PAYLOAD_IE_SHIFT !=
  ...
  while (ptr < mlme_ie_content_offset + ielen &&
         (chTemplate_checkPass == FALSE || tsTemplate_checkpass == FALSE || sync_ie_checkPass == FALSE ||
          slotframelink_ie_checkPass == FALSE)) {
    // subID
    temp16b = *((uint8_t *)(pkt->payload) + ptr);
    temp16b |= (*((uint8_t *)(pkt->payload) + ptr + 1)) << 8;
      ...
      case IEEE802154E_MLME_SLOTFRAME_LINK_IE_SUBID:
        ...
        if (oldFrameLength == 0) {
          ...
          for (i = 0; i < numlinks; i++) {
            ...
            schedule_addActiveSlot(slotoffset,                                     // slot offset
          ...
          energy_throttle_factor = *((uint8_t *)(pkt->payload + ptr + 5 + 5 * numlinks));
        } else {
          numlinks = *((uint8_t *)(pkt->payload + ptr + 4));
          energy_throttle_factor = *((uint8_t *)(pkt->payload + ptr + 5 + 5 * numlinks));
        }
        
        neighbor_addr = &(pkt->l2_nextORpreviousHop);
        neighbors_setNeighborThrottleFactor(neighbor_addr, energy_throttle_factor);
...
void synchronizePacket(PORT_TIMER_WIDTH timeReceived) {
  ...
  if (ieee154e_vars.isSync == TRUE &&
    ...
    LOG_WARNING(COMPONENT_IEEE802154E, ERR_LARGE_TIMECORRECTION, (errorparameter_t)timeCorrection, (errorparameter_t)0);
  }
...
void synchronizeAck(PORT_SIGNED_INT_WIDTH timeCorrection) {
  ...
  if (ieee154e_vars.isSync == TRUE &&
      (timeCorrection < -LIMITLARGETIMECORRECTION || timeCorrection > LIMITLARGETIMECORRECTION)) {
    LOG_WARNING(COMPONENT_IEEE802154E, ERR_LARGE_TIMECORRECTION, (errorparameter_t)timeCorrection, (errorparameter_t)1);
  }
...
void endSlot(void) {
  ...
  if (schedule_getSlottOffset() == ieee154e_vars.slotOffset && schedule_getIsAutoCell() &&
    ...
    if (openqueue_macGetUnicastPacket(&slotNeighbor) == NULL) {
      schedule_removeActiveSlot(ieee154e_vars.slotOffset, CELLTYPE_TX, TRUE, &slotNeighbor);
    }
  }
